...
outName="e2_FSCs_for_iter_${iterName}.png" 
...
gnuplot	<<- EOF
set title "Half-map FSC: Iteration ${iterName}"
...
set style line 1 lt 5 lw 1 pt 7 ps 1 lc rgb "black" 	#Lines
set style line 2 lt 5 lw 3 pt 7 ps 1 lc rgb "gold"  	#unmasked
set style line 3 lt 5 lw 3 pt 7 ps 1 lc rgb "magenta"	#masked
set style line 4 lt 5 lw 3 pt 7 ps 1 lc rgb "cyan"	#tight

set arrow 1 ls 1 from graph 0,first 0.143 to graph 1,first 0.143 nohead lt 7 lw 1 lc "black" front

set output "${outName}"

plot "FSCs_${iterName}_plottable.dat" using 1:2 title ''.word(labels,1).'' with linespoints ls 2, \
     "FSCs_${iterName}_plottable.dat" using 1:3 title ''.word(labels,2).'' with linespoints ls 3, \
     "FSCs_${iterName}_plottable.dat" using 1:4 title ''.word(labels,3).'' with linespoints ls 4
EOF
