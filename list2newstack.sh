...
if [[ -z $1 ]] || [[ -z $2 ]] ; then
	...
	echo "(1) = path/to/alignavers"
	echo "(2) = output stacked file"	
	echo ""
...
ls -rtl ${alignaver} | awk '{print $9}' > tmpNames.txt			

wc -l tmpNames.txt | awk '{print $1}' > $imagestack.txt 						


#for loop to prepare image names the way newstack likes them
for i in `cat tmpNames.txt`; do 
	echo $i
	echo $i >> $imagestack.txt
    echo / >> $imagestack.txt
done
...
newstack -fileinlist ${oriDir}/$imagestack.txt -output ${oriDir}/$imagestack.mrc


echo ""
echo "*********************************************************************************************"
echo "Number of images:" $(wc -l tmpNames.txt | awk '{print $1}')
echo "Wrote out image name file for newstack as:" $imagestack.txt 
echo "Wrote out reconstructed tilt-series file for IMOD:" $imagestack.mrc
echo "*********************************************************************************************"
echo ""
